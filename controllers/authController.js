...
export const loginController = async (req, res, next) => {
    ...
    console.log(`DEBUG OTP for ${user.email}: ${otp}`); // For testing purposes
    await redisClient.set(`otp:${user.email}`, otp, 'EX', 120); // OTP valid for 2 minutes
    ...
    const html = `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; background: #f6f8fa; padding: 32px;">
            <div style="max-width: 400px; margin: auto; background: #fff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.07); padding: 32px;">
                <h2 style="color: #2d3748; text-align: center; margin-bottom: 16px;">Login Verification</h2>
                <p style="font-size: 16px; color: #4a5568; text-align: center;">Use the code below to complete your login for <b>Multi-AI Agent Chatbot</b>:</p>
                ...
                <p style="font-size: 14px; color: #718096; text-align: center;">This OTP is valid for 2 minutes.</p>
    ...
    await sendEmail(user.email, "Login Verification Code", html);
...
export const otpVerifyController = async (req, res, next) => {
    ...
    const { otp, phoneNo, email } = result.data;
    ...
    if (phoneNo) {
        ...
        return next(new BadRequestException("Either email or phone number is required", ErrorCodes.ALL_FIELDS_REQUIRED));
    }

    const storedOtp = await redisClient.get(key);
    if (!storedOtp) {
        return next(new BadRequestException("OTP expired or not sent", ErrorCodes.INVALID_OTP));
    }
